...
allowed-tools: ["Read", "Glob", "Grep", "Bash", "Task", "mcp__supermemory__add_memory", "mcp__plugin_supermemory_supermemory__add_memory", "mcp__claude_ai_supermemory__add_memory"]
---
...
After exploring all phases, save several focused memories — architecture, conventions, key files / how to run — not one giant dump.

Call the supermemory `add_memory` tool (`mcp__supermemory__add_memory` and/or `mcp__plugin_supermemory_supermemory__add_memory` depending on connection). Omit `containerTag` so the MCP proxy scopes the write to this repo.

Confirm with: `Codebase indexed — [N] memories saved about [project name]`
...
- Save several focused memories at the end, not one dump
